...
select Usuario.Nome, Evento.NomeEvento, Situacao, Instituicao.NomeFantasia, DataEvento, Descricao,
( CASE WHEN AcessoLivre = 1 THEN 'Púbico' ELSE 'Privado' END) AS TipoAcesso from Presenca
...
SELECT
    CASE WHEN AcessoLivre = 1 THEN 'Púbico' ELSE 'Privado' END AS AcessoLivre FROM Evento
